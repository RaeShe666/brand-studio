...
export function registerIpcHandlers(
	...
	async function requestScreenAccess() {
		...
		try {
			...
			// TCC prompt without opening Brand Studio's source selector above it.
		...
		if (!access.granted) {
			if (process.platform === "darwin" && access.status !== "not-determined") {
				...
				const messageOptions = {
					...
					detail:
						"Allow Brand Studio in macOS System Settings, then come back and choose a screen or window.",
